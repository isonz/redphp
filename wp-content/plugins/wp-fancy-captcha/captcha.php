...
function captcha_footer() {
	...
	if ( (is_single()||is_page()) && !is_user_logged_in())
	{
        ...
        $notice = $options['Notice']?$options['Notice']:'下面是评论验证码，请用鼠标将<br> %item% 拖进右边的圆框中即可.';
        $text = str_ireplace('%item%','<span></span>',$notice);
        $pencil = $options['Pencil']?$options['Pencil']:'铅笔';
        $scissors = $options['Scissors']?$options['Scissors']:'剪刀';
        $clock = $options['Clock']?$options['Clock']:'时钟';
        $heart = $options['Heart']?$options['Heart']:'爱心';
        $note = $options['Note']?$options['Note']:'音乐';
		?>
...
function captcha_filter($comment) {
	...
	if(is_user_logged_in() || ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['captcha']) && $_POST['captcha'] == $_SESSION['captcha']) || $comment['comment_type'] == 'pingback' || $comment['comment_type'] == 'trackback')
	{
		...
		wp_die( ($options['Error']?$options['Error']:'请拖入正确的图案到圆形验证码区域！').'<input onclick="javascript:history.go(-1)" type="button" value="'.($options['Back']?$options['Back']:'返回').'" />');
	}
}
...
function fancycaptcha_admin_init(){
    ...
    add_settings_field('Error', '请拖入正确的图案到圆形验证码区域！', 'fancycaptcha_Error', 'fancycaptcha', 'fancycaptcha_main');
...
$options = get_option( 'fancycaptcha_options' );
?>
<input type="text" name="fancycaptcha_options[Error]" id="Error" value="<?php echo $options['Error']?$options['Error']:'请拖入正确的图案到圆形验证码区域！'; ?>" /><br />
<?php
}
